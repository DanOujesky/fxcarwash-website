    ...
    try {
        ...
        for (const group of cardGroups) {
            ...
            if (group.status === "IN_STOCK")
                cardStats.inStock = count;
            else if (group.status === "ASSIGNED")
                cardStats.assigned = count;
            else if (group.status === "BLOCKED")
                cardStats.blocked = count;
            else if (group.status === "LOST")
                cardStats.lost = count;
        }
        ...
        for (const group of orderGroups) {
            ...
            if (group.status === "PAID") {
                ...
                orderStats.refunded = count;
            else if (group.status === "SHIPPED")
                orderStats.shipped = count;
        }
...
export const seedLegacyNews = async (req, res) => {
    try {
        ...
        if (existing > 0)
            return res.json({ skipped: true, message: "DB již obsahuje novinky" });
...
export const reclaimCard = async (req, res) => {
    try {
