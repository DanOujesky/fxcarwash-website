...
export const refreshCards = async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        const cards = await fetchNayax(`/operational/v1/cards?CardEmail=${encodeURIComponent(user.email ?? "")}`, { method: "GET" });
        ...
        logger.info({ userId: user.id, count: updatedCards.length }, "Karty synchronizovány z Nayax");
...
export const toggleCardStatus = async (req, res) => {
    try {
        ...
        logger.info({ cardNumber, currentStatus: card.status, newStatusCode }, "Změna stavu karty v Nayax");
        const nayaxResponse = await fetchNayax(`/operational/v1/cards/${card.identifier}/status/${newStatusCode}`, { method: "POST" });
        if (!nayaxResponse || nayaxResponse.Ok !== true) {
            throw new Error(`Nayax API failed: ${nayaxResponse?.Message || "Unknown error"}`);
        }
        ...
        logger.info({ cardNumber, newStatus: updatedCard.status }, "Stav karty úspěšně změněn");
        res.json({
